...
const tyrannosaurus = {name:"tyrannosaurus", diet:"carnivorous", weight:"700kg", length:"12m", period: "Late Cretaceous", roar: function(){return "RAWERSRARARWERSARARARRRR!"} };
...
zooAnimals.forEach((zoo) => {
  displayNames.push(`Name: ${zoo.animal_name}, Scientific: ${zoo.scientific_name_name}`);
});
...
zooAnimals.map((zoo) => {
  lowCaseAnimalNames.push(`${zoo.animal_name}`.toLowerCase());
});
...
lowPopulationAnimals.object = zooAnimals.filter((zoo) => {
...
populationTotal = zooAnimals.reduce((accumulator, zoo) => {
